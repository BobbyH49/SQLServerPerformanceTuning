...
DECLARE
	...
	, @total_page_server_reads_priority_threshold BIGINT = 1024 -- GB
	...
	FROM (
		SELECT TOP 10
			...
			, score = (11 - RANK() OVER(ORDER BY SUM(avg_page_server_io_reads * count_executions) DESC))
				* CASE WHEN SUM(avg_page_server_io_reads * count_executions) / 128 / 1024 >= @total_page_server_reads_priority_threshold THEN 2 ELSE 0 END
		FROM ##QueryStorePerf
		WHERE start_time >= @start_time
		AND end_time <= @end_time
		GROUP BY
