...
AS (
	SELECT
		...
		, avg_page_server_reads_mb = SUM(avg_page_server_io_reads * count_executions) / SUM(count_executions) / 128
...
SELECT
	...
	, page_server_reads_pct =
		CASE total_page_server_reads_gb WHEN 0 THEN 0 ELSE
			CAST(CAST(total_page_server_reads_gb AS DECIMAL(15,2)) * 100 / (
