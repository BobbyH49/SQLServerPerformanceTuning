...
begin
	select
		...
		, stats_columns = substring(convert(nvarchar(4000), ((
			...
			left join sys.index_columns ic on ic.object_id = sc.object_id and ic.index_id = sc.stats_id and ic.index_column_id = sc.stats_column_id and ic.column_id = sc.column_id
			where sc.object_id = s.object_id and sc.stats_id = s.stats_id
			order by case when ic.object_id is null then sc.stats_column_id else ic.key_ordinal end
			for xml path(N'')
		  ))), 2, 4000)
		, sp.last_updated
		, sp.rows
